...
namespace WPT_DiviForms_Divi_Modules\CheckboxesField;

use  ET_Builder_Module ;
class CheckboxesField extends ET_Builder_Module
{
    public  $slug = 'wpt_form_checkboxes_field' ;
    ...
    protected  $module_credits = array(
        'module_uri' => 'https://wptools.app/wordpress-plugin/divi-form-builder-with-material-design/?utm_source=checkbox-field&utm_medium=divi-module&utm_campaign=divi-forms&utm_content=module-credits',
        'author'     => 'WP Tools (7-day FREE Trial)',
        'author_uri' => 'https://wptools.app/wordpress-plugin/divi-form-builder-with-material-design/?utm_source=checkbox-field&utm_medium=divi-module&utm_campaign=divi-forms&utm_content=module-credits',
    ) ;
    /**
     * init divi module *
     */
    ...
    {
        $this->name = esc_html__( 'Checkbox - MD', 'forms-for-divi' );
        $this->icon_path = $this->container['plugin_dir'] . '/resources/images/check-box.svg';
    }
    ...
    public function render( $attrs, $content = null, $render_slug = null )
    {
        ...
        $responsive->generate_responsive_css(
            ...
            '--wpt-form-checkbox-columns',
            $this->slug,
            '',
            'color'
        );
        $checkboxes_field = $this->container['checkboxes_field']->set_name( $name )->set_label( $label )->set_default( $default )->set_helper_text( $helper_text )->set_layout( $layout )->set_options( $options );
        ...
        if ( $field_required ) {
            ...
            $checkboxes_field->add_constraint( 'required', [
                'error_message' => $field_required_message,
            ] );
            $checkboxes_field->set_is_required( true );
        }
        
        return $checkboxes_field->html();
    }
